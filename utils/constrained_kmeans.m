function varargout = constrained_kmeans(X, k, dx, dy, varargin)
    ...
    function cellout = loopBody(rep,S)
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                [C(:,changed), m(changed)] = gcentroids(X, idx, changed, distance, dx, dy);
                    ...
                    switch emptyact
                        ...
                        case 'singleton'
                            for i = empties
                                ...
                                [C(:,from), m(from)] = gcentroids(X, idx, from, distance, dx, dy);
...
%------------------------------------------------------------------
function [centroids, counts] = gcentroids(X, index, clusts, dist, dx, dy)
...
centroids(1,:)*counts'*dx*dy
% update according to the constraint
centroids(1,:) = centroids(1,:)  ./ ( centroids(1,:)*counts'*dx*dy );
